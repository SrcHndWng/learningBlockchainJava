...
public class Transaction {
    ...
    public boolean processTransaction() {
        ...
        for (TransactionInput i : inputs) {
            i.UTXO = Application.UTXOs.get(i.transactionOutputId);
        }
        ...
        outputs.add(
                new TransactionOutput(
                        this.reciepient, value, transactionId)); // send value to recipient
        outputs.add(
                new TransactionOutput(
                        this.sender,
                        leftOver,
        ...
        for (TransactionOutput o : outputs) {
            Application.UTXOs.put(o.id, o);
        }

        // Remove transaction inputs from UTXO lists as spent:
        for (TransactionInput i : inputs) {
            if (i.UTXO == null) continue; // if Transaction can't be found skip it
            Application.UTXOs.remove(i.UTXO.id);
        }
    ...
    public float getInputsValue() {
        ...
        for (TransactionInput i : inputs) {
            if (i.UTXO == null)
                continue; // if Transaction can't be found skip it, This behavior may not be
            // optimal.
            total += i.UTXO.value;
        }
        return total;
